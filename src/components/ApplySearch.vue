      ...
      <div class="result-box">
        ...
        <div class="matchs" v-for="(item, index) in searchResult" :key="index">
          ...
          <table class="result-list">
            ...
            <tr>
              <td class="td-title">衣服尺码：</td>
              <td class="td-value">{{item.dressSize}}</td>
            </tr>
            <tr>
              <td class="td-title">参赛费用：</td>
              <td class="td-value">{{item.price}}</td>
            </tr>
          </table>
        </div>
    ...
    methods: {
      onSubmit() {
        ...
        if (this.name && this.number) {
          fetchHttp('marathon/signUp/othersInformation', { name: this.name, number: this.number }, 'GET').then(
            res => {
              if (res && res.code === 0) {
